# GetJingDongComment(京东评论爬取器V1)
>PS: 前两天做一个项目涉及到爬取京东的评论数据，本来是打算使用Selenium，但因为不明原因被拦截拼尽全力无法战胜，所以还不如直接用JS

**<font color="#EA1B25">注意：本文图片所在的服务器使用ipv6连接，请不要使用校园网/挂梯子</font>**

## 一.使用方法
### 1.提前安装好TemperMonkey油猴拓展
自行搜索自己使用的浏览器的安装教程哦，推荐使用Chrome或Edge
### 2.导入Scripts.js文件
导入后会在京东商品详情页自动加载
![](https://pic1.imgdb.cn/item/681a300558cb8da5c8e14ca8.png)
### 3.爬取评论
#### ①点击开始爬取即可
脚本会自动打开评论窗口并开始爬取评论内容
#### ②停止爬取/直接下载
如果您已经获取到了足够的评论，可以直接点击下载文件下载当前获取到的评论`(.csv)`。也可以停止后再下载。**下载文件功能爬取中/停止均随时可用**
### 4.隐藏UI窗口
按键盘上的`Q键`可以收起/展开UI界面
> 点击按钮没有用，因为我懒得写这个功能
...
导致只能获取固定元素
**解决方案**：应该遍历行内容，找到分隔符（可用类名辨别），然后读取分隔符后面的元素内容
### 2.CSV可能会下载后不完整
**原因不明**：已排除不可见字符、换行等特殊字符的问题，怀疑是AI写的代码有问题，CSV没用库，让AI随手写的转换代码
**解决方案**: 可手动打开调试器拿到原始数组，自行转化为CSV

## 三.随便写的
> 希望大家做数据分析什么的能用得上哈哈哈哈
